...
public sealed class AssignmentService : IAssignmentService
{
    ...
    public async Task<AssignmentDto> UnassignAsync(Guid assignmentId, CancellationToken ct = default)
    {
        ...
        // Unconditional, not just when Status == Assigned: this is also the recovery path for an
        // asset whose status was changed away from Assigned (e.g. to Broken) before that flow
        // auto-closed assignments, or for any other legacy data drift — returning an asset always
        // means it's available again, regardless of what its status happened to say beforehand.
